...
import KinderHomePage from './kinder-home';
import { useProfilesStore } from '@/lib/store/profiles';
import { LEHRPLAN_SUBJECTS } from '@/lib/constants/lehrplan';
import { calculateAge, getZyklus, getApproximateKlasse } from '@/lib/utils/profile-helpers';
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    if (mode === 'learn' && subjectId && activeProfileId) {
      ...
      if (activeProfile && subject) {
        const age = calculateAge(activeProfile.geburtsdatum);
        ...
        useUserProfileStore.getState().setBio(sysMsg);
        ...
        setForm(prev => {
          if (!prev.requirement || prev.requirement === '') {
            ...
            return { ...prev, language: 'de-DE' as any, requirement: defaultReq };
          }
          return prev;
        });
        ...
        if (pdfPath) {
          fetch(pdfPath)
            .then(res => res.blob())
            .then(blob => {
              const file = new File([blob], `${subject.name}_Lehrplan.pdf`, { type: 'application/pdf' });
  ...
  }, [searchParams, activeProfileId, profiles]);
...
export default function Page() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-background flex flex-col items-center justify-center">Laden...</div>}>
      <PageContent />
    </Suspense>
  );
}
